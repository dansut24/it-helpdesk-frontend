...
const RaiseIncidentForm = ({ renameTabAfterSubmit }) => {
    ...
    try {
      ...
      if (refNum && id) {
        if (renameTabAfterSubmit) {
          renameTabAfterSubmit("New Incident", refNum, refNum);
        }
      } else {
      ...
      if (renameTabAfterSubmit) {
        renameTabAfterSubmit("New Incident", simulatedReference, simulatedReference);
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <Box p={2} maxWidth={800} mx="auto" position="relative">
      {/* Progress Bar */}
      <LinearProgress
        variant="determinate"
        value={step === 1 ? 50 : 100}
      ...
      <Fade in={step === 1} timeout={400} unmountOnExit>
        <Box>
          <Paper elevation={2} sx={{ p: 0, borderRadius: 2, overflow: "hidden", mb: 3 }}>
